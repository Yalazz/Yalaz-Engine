...
void VulkanEngine::draw_wireframe_outline(VkCommandBuffer cmd, const RenderObject& obj, VkDescriptorSet descriptor)
{
...
void VulkanEngine::draw_viewing(VkCommandBuffer cmd)
{
    // Grid kontrolü: Her modda grid açılıp kapatılabilir
    if (_showGrid)
    {
        draw_grid(cmd, globalDescriptor);
    }
...
void VulkanEngine::update_scene() {
    ...
    // Cache light sphere mesh on first use
    static GPUMeshBuffers cachedLightSphereMesh;
    static bool lightMeshCached = false;
    if (!lightMeshCached) {
        cachedLightSphereMesh = generate_sphere_mesh();
        lightMeshCached = true;
    }
    ...
    if (scenePointLights.size() != lastLightCount) {
        ...
        for (const auto& light : scenePointLights) {
            StaticMeshData sphere;
            sphere.mesh = cachedLightSphereMesh;  // Use cached mesh!
            sphere.position = glm::vec3(light.position);
            sphere.scale = glm::vec3(0.25f);
            sphere.materialType = ShaderOnlyMaterial::EMISSIVE;
            sphere.passType = MaterialPass::MainColor;
...
void VulkanEngine::init_descriptors()
{
    ...
    // b) Scene data + bindless texture array için (set = 0)
    {
        ...
        builder.add_binding(1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1024, true);          // allTextures[]
        _gpuSceneDataDescriptorLayout = builder.build(_device, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);
    }
